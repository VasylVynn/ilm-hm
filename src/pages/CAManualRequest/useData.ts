...
export const useData = () => {
  ...
  const serverStatusText = useMemo(() => {
    ...
    if (serverStatus.memberTaskStatus) {
      return `Сервер зайнятий. Попередні товари C&A ще не оброблені.`;
    }
  ...
  const checkServerStatus = () => {
    axios.get('https://ca.wizz-app.net/api/jobsStatus')
      .then(response => {
  ...
  //   axios.get('https://ca.wizz-app.net/api/checkMemberCount')
  //     .then(response => {
  ...
  const requestScraping = ({ url }: requestScrapingProps) => {
    axios.post('https://ca.wizz-app.net/api/startScraping',
      {
        url,
      })
      .then((response) => {
  ...
  //   axios.post('https://ca.wizz-app.net/api/stopScraping')
  //     .then((response) => {
  //       if (response.status === 200) {
  //         setStatus('stopped')
  //       } else {
